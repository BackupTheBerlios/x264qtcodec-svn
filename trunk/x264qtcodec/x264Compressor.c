...
static ComponentResult emit_frame_from_nal(x264_compressor_globals_t *glob, x264_nal_t *nal, unsigned int nal_count, x264_picture_t *picture)
{
    ...
    for (i = 0; i < nal_count; i++) {
        int encoded_size = x264_nal_encode(frame_data_ptr, &buffer_space_left, 1, &nal[i]);
...
ComponentResult x264_compressor_RequestSettings(x264_compressor_globals_t *glob, Handle settings, Rect *rp, ModalFilterUPP filter_proc)
{
	...
	SetControl32BitValue(control, glob->params.b_bframe_adaptive);
	
	/* B-frame Pyramids Checkbox */
	control_id.id = 129;
  	GetControlByID(window, &control_id, &control);
	SetControl32BitValue(control, glob->params.b_bframe_pyramid);
  														...
  														CFDictionaryRef *compressor_pixel_buffer_attributes_out)
{
	...
	*compressor_pixel_buffer_attributes_out = pixel_buffer_attributes;
    ...
    for (i = 0; i < sps_count; i++) {
        ...
        /* Encode the SPS NAL */
        buffer_size = x264_nal_encode(sps_buffer, &buffer_size, 1, &nal[sps_index]);
...
ComponentResult x264_compressor_CompleteFrame(x264_compressor_globals_t *glob, ICMCompressorSourceFrameRef source_frame, UInt32 flags)
{
    ...
    while (CFArrayGetCount(glob->source_frame_queue)) {
        ...
        x264err = x264_encoder_encode(glob->encoder, &nal, &nal_count, NULL, &picture);
        if (x264err < 0) {
            err = x264err;
			return err;
        }
